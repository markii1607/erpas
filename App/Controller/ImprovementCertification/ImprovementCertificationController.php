<?php
    ...
    require_once("../../Model/ImprovementCertification/ImprovementCertificationQueryHandler.php");

    use App\Config\BaseController as BaseController;
    use App\Model\ImprovementCertification\ImprovementCertificationQueryHandler as QueryHandler;
    use Exception;

    class ImprovementCertificationController extends BaseController {
        /**
        ...
        public function getLotOwners($lot_no)
        {
            $data = [
                'is_active' => 1,
                'status'    => 1,
                'lot_no'    => $this->strLike($lot_no),
            ];
        ...
        public function getDBImprovementRecords($lot_no)
        {
            $data = [
                'is_active'         => 1,
                'status'            => 1,
            ...
            $records = $this->dbCon->prepare($this->queryHandler->selectDBImprovementRecords()->end());
            $records->execute($data);

            return $records->fetchAll(\PDO::FETCH_ASSOC);
        }

        public function archivePropTaxDec($input)
        {
            try {
        ...
        public function getReleasedCertifications($id = '')
        {
            ...
            foreach ($result as $key => $value) {
                $result[$key]['location']    = (!empty($value['prop_location_street']) ? $value['prop_location_street'].' ' : '').$value['brgy_name'];
