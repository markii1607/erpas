<?php 
    ...
    class SupplyReportTotalQuantityQueryHandler extends QueryHandler{
        ...
        public function selectPRSD($prsId = false, $request_table = '', $dateFrom = false, $dateTo = false, $catId = false, $specsId = false, $unit = false, $charging = false, $requestor = false, $pm = false, $prdstat = false)
        {
            $fields = array(
                'PRD.id',
                ...
                'MS.category_id as ms_mat_category_id'

            );


            // ($request_table == "purchase_requisition_descriptions") ?    array_push($fields, 'PRD.material_specification_id') :  array_push($fields, 'PRD.material_specification_id as item_spec_id');
            // ($request_table == "purchase_requisition_descriptions") ?    array_push($fields, 'PRD.item_spec_id') : '';
            ...
            if($request_table == "purchase_requisition_descriptions"){
              
                array_push($fields, 'PRD.purchase_requisition_id');
                array_push($fields, 'PRD.material_specification_id');
                array_push($fields, 'PRD.item_spec_id');
                array_push($fields, 'PRD.pm_id');
                array_push($fields, 'PRD.unit_measurement');
                
              
                $join['material_specifications MS'] = 'MS.id = PRD.item_spec_id';

            }else if($request_table == "prs_ppe_descriptions"){
                array_push($fields, 'PRD.purchase_requisition_id');
                array_push($fields, 'PRD.material_specification_id as item_spec_id');
                array_push($fields, 'PRD.pm_id');
                array_push($fields, 'PRD.unit_measurement');

                $join['material_specifications MS'] = 'MS.id = PRD.material_specification_id';

            }else{
                array_push($fields, 'PRD.pr_id as purchase_requisition_id'); 
                array_push($fields, 'PRD.material_specification_brand_id');
                array_push($fields,  'PRD.unit_of_measurement as unit_measurement');
            }
        
            // $join['material_specifications MS'] = ($request_table == "purchase_requisition_descriptions") ?   'MS.id = PRD.item_spec_id' : 'MS.id = PRD.material_specification_id';
            ...
            if($request_table == "pr_medical_materials"){
                $initQuery = ($prsId)              ? $initQuery->andWhere(array('PRD.pr_id'   => ':prs_id'))       : $initQuery; 
                $initQuery = ($catId)              ? $initQuery->andWhere(array('MS.category_id' => ':material_category_id'))        : $initQuery; 
    
                $specsColumnHead = ($request_table == "purchase_requisition_descriptions") ?   'PRD.item_spec_id' : 'PRD.material_specification_id';
                $initQuery = ($specsId)            ? $initQuery->andWhere(array($specsColumnHead => ':material_specification_id'))   : $initQuery; 
    
                $initQuery = ($unit)               ? $initQuery->andWhere(array('PRD.unit_of_measurement'          => ':unit'))      : $initQuery; 
                $initQuery = ($dateFrom)           ? $initQuery->logicEx('AND 5.created_at  >= :date_from')                        : $initQuery;
        ...
        {   
            $fields     = array(
                'M.cost_code as m_material_cost_code',
