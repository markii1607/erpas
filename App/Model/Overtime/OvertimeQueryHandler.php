<?php 
    namespace App\Model\Overtime;

    require_once("../../AbstractClass/QueryHandler.php");

    use App\AbstractClass\QueryHandler;

    class OvertimeQueryHandler extends QueryHandler { 
        /**
         * `selectOvertimes` Query string that will fetch overtime.
         * @return string
         */
        public function selectOvertimes($id = false, $piId = false)
        {
            $fields = [
                'O.id',
            ...
            $initQuery = ($piId) ? $initQuery->andWhere(['O.personal_information_id' => ':personal_information_id']) : $initQuery;
   ...
   /**
        ...
        public function selectPersonalInformations($id = '')
        {
            ...
            $initQuery = $this->select($fields)
                              ->from('users U')
                              ->join($joins)
                              ->where(['U.is_active' => ':is_active', 'PI.is_active' => ':is_active', 'EI.is_active' => ':is_active']);
        ...
        public function updateOvertimeSignatories($id = '', $data = [])
        {
