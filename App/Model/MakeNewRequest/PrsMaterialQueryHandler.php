...
class PrsMaterialQueryHandler extends MakeNewRequestQueryHandler
{
    ...
    public function selectTotalRequestQuantity($category = false, $pm_id = false)
    {
        $fields = array(
            // 'SUM(PRD.quantity) as total_requested_quantity'
            'PR.id as pr_id',
            'PR.status as pr_status',
            'PR.for_cancelation as pr_for_cancelation',
            'PR.is_active as pr_is_active',
            'PR.project_id as pr_project_id',
            'PRD.id as prd_id',
            'PRD.pm_id prd_pm_id',
            'PRD.quantity prd_quantity',
            'PRD.status prd_status',
            'PRD.category prd_category',
        ...
        $initQuery = ($category) ? $initQuery->andWhere(array('PRD.category' => ':category')) : $initQuery;
        $initQuery = ($pm_id) ? $initQuery->andWhere(array('PRD.pm_id' => ':pm_id')) : $initQuery;
        // $initQuery = $initQuery->logicEx('AND PRD.status BETWEEN 1 AND 9');
        // $initQuery = $initQuery->logicEx('AND PR.status BETWEEN 2 AND 5 AND PR.for_cancelation IS NULL OR PR.for_cancelation = 0 AND PRD.status BETWEEN 1 AND 9');
