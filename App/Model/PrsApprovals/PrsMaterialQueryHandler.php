...
class PrsMaterialQueryHandler extends PrsApprovalsQueryHandler
{
     ...
     * @return void
     */
    public function selectPrss($id = false, $user_id = "", $request_type = false, $category = "", $filter = false, $limit = 0)
    {
        $fields = array(
            'PR.id',
            'PR.project_id',
        ...
        // PR.signatories REGEXP '\"user_id\":\"13\"(?:(?!a-zA-Z).)*?\"status\":2'
        switch ($category) {
            case 'Approved PRS':
                $initQuery = $initQuery->logicEx("AND PR.signatories REGEXP '\"user_id\":\"" . $user_id . "\"(?:(?!a-zA-Z)[^\{])*?\"status\":2'");
                break;
            case 'Disapproved PRS':
                $initQuery = $initQuery->logicEx("AND PR.signatories REGEXP '\"user_id\":\"" . $user_id . "\"(?:(?!a-zA-Z)[^\{])*?\"status\":1'");
                break;
            case 'Cancellation Approvals':
                $initQuery = $initQuery->logicEx("AND PR.for_cancelation = 0 AND PR.head_id = " . $user_id);
                break;
            default:
                $initQuery = $initQuery->logicEx("AND PR.for_cancelation IS NULL AND PR.status = 2 AND ( PR.signatories LIKE '%\"current_id\":\"" . $user_id . "\"}' OR PR.signatories LIKE '%\"current_id\":" . $user_id . "}' )");
                break;
        }
        // $initQuery = $initQuery->logicEx(' ORDER BY PR.prs_no DESC');
        $initQuery = $initQuery->logicEx('ORDER BY PR.prs_no DESC LIMIT ' . $limit . ', 25');
    ...
    public function selectRequestItems($id = false, $prsId = false, $mode = false)
    {
        $fields = array(
            ...
            'PRD.material_id',
            'PRD.expense_type',
            'PRD.expense_type as expense_type_holder',
            'PRD.category',
            'PRD.wi_category',
            'PRD.work_item_id',
            'PRD.work_volume',
    ...
    public function updateRequestSequence($id = '', $data = array())
    {
        $initQuery = $this->update('prd_delivery_sequences', $id, $data);
        return $initQuery;
    }
