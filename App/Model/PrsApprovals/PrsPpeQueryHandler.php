...
class PrsPpeQueryHandler extends PrsApprovalsQueryHandler
{
  /**
   * selectPrss
   *
  ...
  public function selectPrss($id = false, $request_type = false)
  {
    $fields = array(
      ...
      'PR.project_id',
      'PR.department_id',
      'IF(PR.project_id IS NULL, "1", "0") as charge_to',
      'PR.prs_no',
      'PR.request_type_id',
      'PR.for_cancelation',
      'PR.signatories',
  ...
  public function selectDeliverySequence($id = false, $prdId = false)
  {
    ...
    $initQuery = $this->select($fields)
      ->from('prs_ppe_delivery_sequences PRDS')
      ->where(array('PRDS.is_active' => ':is_active'));

    $initQuery = ($id) ? $initQuery->andWhere(array('PRDS.id' => ':id')) : $initQuery;
    $initQuery = ($prdId) ? $initQuery->andWhere(array('PRDS.prs_ppe_description_id' => ':prd_id')) : $initQuery;
  ...
  public function selectAttachments($id = false, $preId = false)
  {
    ...
    $initQuery = $this->select($fields)
      ->from('prs_ppe_attachments PRPPEA')
      ->where(array('PRPPEA.is_active' => ':is_active'));

    $initQuery = ($id) ? $initQuery->andWhere(array('PRPPEA.id' => ':id')) : $initQuery;
    $initQuery = ($preId) ? $initQuery->andWhere(array('PRPPEA.prs_ppe_description_id' => ':pre_id')) : $initQuery;
