...
class PrsHeavyEquipmentQueryHandler extends PrsApprovalsQueryHandler
{
  /**
   * selectPrss
   *
  ...
  public function selectAttachments($id = false, $preId = false)
  {
    ...
    $initQuery = $this->select($fields)
      ->from('pr_heavy_equipment_attachments PRHEA')
      ->where(array('PRHEA.is_active' => ':is_active'));

    $initQuery = ($id) ? $initQuery->andWhere(array('PRHEA.id' => ':id')) : $initQuery;
    $initQuery = ($preId) ? $initQuery->andWhere(array('PRHEA.pr_equipment_id' => ':pre_id')) : $initQuery;
