<?php
    namespace App\Model\RequisitionField;

    require_once("../../AbstractClass/QueryHandler.php");

    use App\AbstractClass\QueryHandler;

    class RequisitionFieldQueryHandler extends QueryHandler {
        ...
        public function selectRequisitionField($id = false, $prsId = false)
        {
            ...
            $initQuery = $this->select($fields)
                              ...
                              ->join($joins)
                              ->leftJoin($leftJoins)
                              ->where(['PR.status' => ':status', 'PR.request_type_id' => ':request_type_id', 'PR.created_by' => ':created_by', 'PL.category' => ':category'])
                              ->andWhereNull(array('PR.for_cancelation', 'PR.department_id'));
        ...
        public function selectPrLabors($id = false, $prsId = false)
        {
            ...
            $initQuery = $this->select($fields)
                              ->from('pr_labors PL')
                              ->join($joins)
                              ->where(['PL.is_active' => ':is_active', 'PL.category' => ':category']);
        ...
        /**
         * `selectRequisitionFieldLabor` Query string that will fetch RequisitionField.
         * @return string
         */
