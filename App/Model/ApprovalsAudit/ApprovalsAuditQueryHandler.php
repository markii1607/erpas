...
namespace App\Model\ApprovalsAudit;

require_once('../../AbstractClass/QueryHandler.php');

use App\AbstractClass\QueryHandler;

class ApprovalsAuditQueryHandler extends QueryHandler
{
    ...
    public function selectAuditMonitoring($id = false, $filterVal = false)
    {
        $fields = array(
            ...
            'AM.created_by',
            'AM.updated_by',
            'DATE_FORMAT(AM.created_at, "%b %d,%Y %h:%i %p") as created_at',
            'DATE_FORMAT(AM.updated_at, "%b %d, %Y %h:%i %p") as updated_at',
            'AM.is_active',
        );
    ...
    public function selectAOB($id = false, $status = false)
    {
        $fields = array(
            'AOB.id',
            'AOB.aob_no',
            'DATE_FORMAT(AOB.date, "%b %d, %Y %h:%i %p") as date',
            // 'AOB.note',
            'AOB.status',
            'AOB.signatory_level',
            'AOB.is_active',
            ...
            'AOB.created_at',
            // 'AOB.updated_at'
            'DATE_FORMAT(AOB.updated_at, "%b %d, %Y %h:%i %p") as updated_at'
        );
    ...
    public function selectRfqDesc($id = false, $rfq_mat_id = false)
    {
        $fields = array(
            ...
            'RFQD.pr_heavy_equipment_id',
            'RFQD.pr_tool_id',
            'RFQD.pr_equipment_id',
            'RFQD.pr_medical_material_id',
            'RFQD.prs_ppe_description_id',
            'RFQD.is_active',
            'RFQD.created_by',
            // 'RFQD.updated_by',
            'RFQD.created_at',
    ...
    public function selectPrsPpeDescriptions($id = false)
    {
        ...
        $initQuery = $this->select($fields)
            ...
            ->where(array('PPD.is_active' => ':is_active'));

        $initQuery = ($id) ? $initQuery->andWhere(array('PPD.id' => ':id')) : $initQuery;
